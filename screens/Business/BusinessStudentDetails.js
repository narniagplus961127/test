...
class BusinessStudentDetails extends Component{
    constructor(props){
        super(props);
        this.state = {
            Uname:'', UnameValid: false, UnameError: null, UnameDisabled: true,
            sID:'', sIDValid: false, sIDError: null, sIDDisabled: true,
    ...
    onNext(){
        const businessType= this.props.navigation.getParam('businessTypeS');
        ...
        if((this.state.UnameValid && this.state.sIDValid &&
            this.state.nameValid && this.state.icValid && this.state.hpValid
    ...
    validateForm(){
        if((this.state.UnameDisabled && this.state.sIDDisabled &&
            this.state.nameDisabled && this.state.icDisabled && this.state.hpDisabled
    ...
    validateUName(){
        if (this.state.Uname != ''){
            ...
            this.setState({UnameDisabled: false})
        }
        else{
            this.setState({UnameValid: false})
            this.setState({UnameError: 'This field is required.'})
            this.setState({UnameDisabled: true})
        }
    }

    validateSID(){
        if (this.state.sID != ''){
            this.setState({sIDValid: true})
            this.setState({sIDError: null})
            this.setState({sIDDisabled: false})
        }
        else{
            this.setState({sIDValid: false})
            this.setState({sIDError: 'This field is required.'})
            this.setState({sIDDisabled: true})
        }
    }
    ...
    render(){
        const isUnameCorrect = this.state.UnameValid;
        const isSIDCorrect = this.state.sIDValid;
        ...
        return (
            ...
            <View>
                <Text style = {styles.content}>Name of University / College</Text>
                <TextInput
                    style={isUnameCorrect? styles.textinput : styles.textinputerror}
                    placeholder='Enter University Name or College'
                ...
                />
                <Text style = {{color: 'red', marginHorizontal: '5%'}}>{this.state.UnameError}</Text>         

                <Text style = {styles.content}>Student ID</Text>
                <TextInput
                    style={isSIDCorrect? styles.textinput : styles.textinputerror}
                ...
                />
                <Text style = {{color: 'red', marginHorizontal: '5%'}}>{this.state.sIDError}</Text>     
...
export default BusinessStudentDetails;
