...
class BusinessVendorDetails extends Component{
    constructor(props){
        super(props);
        this.state = {
            crn: '', crnValid: false, crnError: null, crnDisabled: true,
    ...
    onNext(){
        const businessType= this.props.navigation.getParam('businessTypeV');
        ...
        if((this.state.crnValid && this.state.nameValid && this.state.icValid && this.state.hpValid
    ...
    validateForm(){
        if((this.state.crnDisabled && this.state.nameDisabled && this.state.icDisabled && this.state.hpDisabled
    ...
    validateCRN(){
        if (this.state.crn != ''){
            this.setState({crnValid: true})
            this.setState({crnError: null})
            this.setState({crnDisabled: false})
        }
        else{
            this.setState({crnValid: false})
            this.setState({crnError: 'This field is required.'})
            this.setState({crnDisabled: true})
        }
    }
    ...
    render(){
        ...
        const isCrnCorrect = this.state.crnValid;
        ...
        return (
            ...
            <View>
                <Text style = {styles.content}>Company Registration Number </Text>
                <TextInput
                    style={isCrnCorrect? styles.textinput : styles.textinputerror}
                    placeholder='Enter Company Registration Number'
                    placeholderTextColor='grey'
                    autoCapitalize = 'none'
                    autoCorrect = {false}
                    onChangeText={(text) => this.setState({crn: text})}
                ...
                />
                <Text style = {{color: 'red', marginHorizontal: '5%'}}>{this.state.crnError}</Text>
